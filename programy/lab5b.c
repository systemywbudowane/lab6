...
int main()
{
  ...
  while(1) {
	  ...
	  if(sw & SW0) {
		  if(room_flags[S1] == 1 && alarm_flag == 0) {
		  ...
		  if(alarm_flag == 1) {
			  alarm_flag = 0;
			  if(alt_alarm_start(&alarm, ENTER_TIME * alt_nticks(), led_on, NULL) < 0)
				  printf("timer error");
		  }
	  }
	  else {
		  if(room_flags[S1] == 0 && alarm_flag == 0) {
			  alt_alarm_stop(&alarm);
			  alarm_flag = 1;
			  room_flags[S1] = 1;
		  }
		  ...
		  if(alarm_flag == 1) {
			  alarm_flag = 0;
			  if(alt_alarm_start(&alarm, EXIT_TIME * alt_nticks(), led_off, NULL) < 0)
				  printf("timer error");
		  }
	  }
